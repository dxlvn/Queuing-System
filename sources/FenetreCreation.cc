...
void FenetreCreation::start()
{
  if(reseau->getNbFiles()>0)
  {
    ...
    bool ok;
    int duree = QInputDialog::getInt(this,"Simulation's period","Please enter the simulation's period desired : ",1,20,100,1,&ok);
    ...
    if(ok){
      ...
      if(reseau->getTypeReseau()==0)
      {
        ...
        //Demander le nombre de clients à entrer
		    int nbClientsInitial = QInputDialog::getInt(this,"Simulation's period","Please enter the simulation's period desired : ",1,1,100,1,&ok);
...
void FenetreCreation::add_queue()
{
  ...
  if(reseau->getTypeReseau()==0)
  {
    //Si les coordonnées du clic correspondent à une file
    while(verif_proximite_clic(vue->getPosXClic(), vue->getPosYClic()))
    {
        fenetre_vue_cliquable->show();
        QMessageBox::information(this,"", "A queue is already present on this location. Please click on a new location.");
        //Réouvre la fenêtre contenant la vue pour récupérer le clic
        fenetre_vue_cliquable->exec();
    }
  }
    ...
    if(res_clic == 1)
    {
      ...
      if((int)(*it_files)->getListeServeur().size() != bcf->recuperer_valeur_newNbServeurs())
      {
        ...
        if((int)(*it_files)->getListeServeur().size() < bcf->recuperer_valeur_newNbServeurs())
        {
          ...
          int nb_serveurs_a_ajouter = bcf->recuperer_valeur_newNbServeurs()-(*it_files)->getListeServeur().size();
        ...
        else if((int)(*it_files)->getListeServeur().size() > bcf->recuperer_valeur_newNbServeurs())
        {
          ...
          int nb_serveurs_a_supprimer = (*it_files)->getListeServeur().size() - bcf->recuperer_valeur_newNbServeurs();

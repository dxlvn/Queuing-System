...
**/
FenetreSimulationRF::FenetreSimulationRF(Reseau * r, Vue * v,QWidget * parent) : QWidget(parent),simulation(r,25), fenetre_infos_reseau(r),fenetre_echeancier(&(simulation.getEcheancier()))
{
  try
  {
    if((r == NULL)||(v == NULL)) throw invalid_argument("Constructeur FenetreSimulationRF : pointeur Reseau et/ou pointeur Vue à NULL");
    else
    {
...
/**Destructeur**/
FenetreSimulationRF::~FenetreSimulationRF()
{
  delete results;
  delete infos_queue;
...
SimulationRF& FenetreSimulationRF::getSimulation()
{
    return simulation;
}
...
HistoriqueRF& FenetreSimulationRF::getHistorique()
{
    return historique;
}
...
FenetreInfosReseau& FenetreSimulationRF::getFenetreInfosReseau(){
  return fenetre_infos_reseau;
}

void FenetreSimulationRF::History()
{
  //Créer la boîte de dialogue de l'historique
  DialogHistoriqueRF * dhRF = new DialogHistoriqueRF(&historique);
...
void FenetreSimulationRF:: Results()
{
  ...
  DialogResultatsRF * drRF = new DialogResultatsRF(&(historique.getHistorique().back()));
...
void FenetreSimulationRF::Infos_Queue()
{
...
void FenetreSimulationRF::Infos_Classes()
{
...
void FenetreSimulationRF::chrono_refresh()
{
...
void FenetreSimulationRF::start_chrono()
{
...
void FenetreSimulationRF::resume(){
  timer_chrono->start(1000);
}

/**Méthode mettant en pause la simualtion**/
void FenetreSimulationRF::pause(){
  timer_chrono->stop();
}

/**Méthode arrêtant la simulation**/
void FenetreSimulationRF::stop(){
  //Envoyer un signal pour arrêter la boucle dans la méthode executer_simulation
  timer_chrono->start(0);
  timer_chrono->stop();
  *chrono = chrono->addSecs(-(secondes));
	secondes = 0;
}
...
void FenetreSimulationRF::executer_simulation(const int nbClientsInitial)
{
  ...
  try{
      if(nbClientsInitial < 0) throw ("Méthode executer_simulation : entier nbClientsInitial négatif");
        ...
        && (!echeancier->getListeEvenement().empty())&&(timer_chrono->isActive()))
        {
...
void FenetreSimulationRF::afficher_compteur_clients()
{
  ...
  for(auto it_files = reseau->getListeFile().begin(); it_files != reseau->getListeFile().end(); it_files++)
  {
    ...
    (vue->scene()->addRect(0,0,LARGEUR_FILE-10,LARGEUR_FILE/2,pen))->setPos(coord_file.x+10,coord_file.y-LARGEUR_FILE);
    ...
    vue->scene()->addRect(rectangle)->setPos(coord_file.x-LONGUEUR_FILE/2,coord_file.y-LARGEUR_FILE/2);
    vue->scene()->addEllipse(0,0,LARGEUR_FILE, LARGEUR_FILE)->setPos(coord_file.x+LONGUEUR_FILE/2, coord_file.y-LARGEUR_FILE/2);

    //Ajoute le rectangle représentant le numéro de la file
    vue->scene()->addRect(0,0,LARGEUR_FILE-10,LARGEUR_FILE/2)->setPos(coord_file.x-LARGEUR_FILE,coord_file.y-LARGEUR_FILE);
    ...
    QGraphicsTextItem * texte = vue->scene()->addText(*id);
